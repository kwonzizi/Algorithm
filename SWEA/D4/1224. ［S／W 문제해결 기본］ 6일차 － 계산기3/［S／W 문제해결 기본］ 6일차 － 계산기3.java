
import java.util.Scanner;
import java.io.FileInputStream;
import java.util.Stack;


class Solution
{
    
    public static int get_prior(char c) {
    	if( c == '*' ) return 1;
        else if( c == '+' ) return 3;
        else if( c == '(' ) return 5;
        else return -1;
          
    
    }
	public static void main(String args[]) throws Exception
	{
	
		Scanner sc = new Scanner(System.in);
        
        	for(int tc = 1; tc <= 10; tc++){
				int T;
				T=sc.nextInt();
                
                String str = sc.next();
                Stack<Character> c_stack = new Stack();
                Stack<Integer> i_stack = new Stack();
                String s = "";
                
                for(int i =0; i < str.length(); i++) {
                    
                    char c = str.charAt(i);
                    
                    if( c -'0' >= 0 && c -'0' <= 9) {
                    	s += String.valueOf(c);
                    }else if(c == ')') {
                            char cur = c_stack.pop();
                            while(!c_stack.isEmpty()&& cur != '(') {
                                s += String.valueOf(cur);
                              	cur = c_stack.pop();                
                            }   
                        }else {

       		 				if(c_stack.isEmpty()) {
                                c_stack.push(c);
                            } else {
                                      if(c == '(') {

                                      c_stack.push(c);
                                   }
                                   else if(get_prior(c_stack.peek()) > get_prior(c)) {

                                          c_stack.push(c);

                                    }else {

                                     	   s += String.valueOf(c_stack.pop());
                                          c_stack.push(c);

                                    }
                              }
                     	 
                    }
                
                }

                while(!c_stack.isEmpty()) {
                	s += c_stack.pop();
                }
                
                for(int j = 0; j < s.length(); j++) {
                    
                    char c = s.charAt(j);
                    
                    if( c-'0' >= 0 && c -'0' <= 9) {
                    	i_stack.push(c-'0');
                        
                    }else {
                        
                        int temp1 = i_stack.pop();
                        int temp2 = i_stack.pop();
                        
                        if(c=='*') i_stack.push(temp1*temp2); 
                        else if(c=='+') i_stack.push(temp1+temp2);
                        
                    }
                
                }
	
			System.out.println("#" + tc + " " + i_stack.pop());
		}
	}
}